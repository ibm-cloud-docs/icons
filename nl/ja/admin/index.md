...
事前承認済み更新期間の設定と、保守に関する特定のブラックアウト日の設定について詳しくは、『[保守の更新情報](index.html#oc_schedulemaintenance)』を参照してください。
...
{{site.data.keyword.Bluemix_notm}} 環境の初期デプロイメント後に、ご使用の環境の管理に使用されるコンポーネントに対して検証チェックが自動的に実行されます。検証検査の実行後に、管理コンソールの「検証チェック」ページに移動して、コンポーネントの状況を確認できます。このページにアクセスするには、<code>https://console.&lt;subdomain&gt;.bluemix.net/check</code> に移動します。ここで、`<subdomain>` は、ローカル・インスタンスまたは専用インスタンスの名前です。
...
![カタログ内のプラン・メタデータの詳細。](images/plan_metadata.png "Bluemix カタログのプラン・メタデータ値ビュー")
...
<p>コンテナーについて詳しくは、『[IBM コンテナーについて](/docs/containers/container_ov.html)』を参照してください。コンテナー割り当て量について詳しくは、『[割り当て量および {{site.data.keyword.Bluemix_notm}} アカウント](/docs/containers/container_planning_org_ov.html#container_planning_quota)』を参照してください。</p>
...
| レポート| **Reports** 許可を持つユーザーには、セキュリティー・レポートを **Read** または **Write** (modify) するアクセス権限を割り当てることができます。「読み取り」のアクセス権限により、ユーザーは「レポートおよびログ」タイルにアクセスして、レポートをダウンロードできます。書き込みアクセスでは、ユーザーは[「レポートおよびログ」](#oc_report)タイルを表示できる他に、CLI を使用して、新規レポートのアップロードおよびユーザーがアクセスする新規カテゴリーの作成も行えます。|
...
4 つの API を使用して、環境、アプリケーション、および組織に関するメトリックを収集できます。これらの API は、指定した期間内に要求したメトリックのデータ・ポイントの配列を返します。
...
4. データは、API 要求が行われた時点から 5 カ月前まで使用可能です。

## 環境に関するメトリックの収集

環境 API を使用して、指定した期間における高度な環境情報を収集できます。およそ毎時記録される実際のデータ・サンプル、またはそれらのデータ・サンプルに基づいて計算された統計を表示できます。使用可能な統計は、メモリー使用量およびディスク使用量の統計のみです。これらの API では、返される実際のデータまたは統計データの時刻範囲を指定することができます。例えば、環境についての 6 時間の CPU (実) データを要求すると、要求された 6 時間の 1 時間ごとの CPU データが応答に含まれます。
...
ご使用環境のメモリーおよびディスクの使用量のメトリック・データに基づいて計算された統計を表示することができます。返される統計は、各データ・グループ内のデータ・サンプルの平均値、最大値、および最小値です。データは、日によってグループ化する (その場合、統計の計算はそれぞれの日を対象にして行われる) か、月によってグループ化する (その場合、統計の計算はそれぞれの月を対象にして行われる) ことができます。
...
以下の照会パラメーターを使用して、ディスクおよびメモリーの統計を収集することができます。

<dl class="parml">
<dt class="pt dlterm">startTime</dt>
<dd class="pd">データを返す最初の時点。startTime が指定されないと、入手可能な最初のデータ・ポイントが含められます。</dd>
<dt class="pt dlterm">endTime</dt>
<dd class="pd">データを返す最後の時点。endTime が指定されないと、最新のデータ・ポイントが使用されます。</dd>
<dt class="pt dlterm">averageType</dt>
<dd class="pd">統計の計算の基になるグループ。有効値は `day` および `month` です。デフォルトは day で、指定された時刻範囲内のそれぞれの日の平均使用量の値を返します。month の値を指定すると、それぞれの月の平均使用量の値が返されます。</dd>
</dl>

以下の例は、照会パラメーターを使用して、日によって平均値が計算された、環境に関するメモリー統計を収集します。
