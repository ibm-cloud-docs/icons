...
{{site.data.keyword.Bluemix_notm}} 有料アカウントでは、組織内で使用された計算、コンテナー、およびサービスに対して課金されます。他の {{site.data.keyword.Bluemix_notm}} ユーザーに招待されて、別のアカウントで組織に参加する場合があります。招待された組織でアプリを作成したりサービスを使用したりする場合、発生した使用量はその組織を含むアカウントに対して課金されます。具体的な課金の詳細な情報は、{{site.data.keyword.Bluemix_notm}} カタログのリソース詳細ページ、または  {{site.data.keyword.Bluemix_notm}} の「料金」ページの料金カリキュレーターで確認できます。

使用しているフィーチャーに応じて、異なるタイプの料金が適用されます。以下の表に、概要を示します。
