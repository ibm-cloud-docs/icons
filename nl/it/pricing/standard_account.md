...
Se il tuo account di prova non viene convertito, viene visualizzato un messaggio che ne spiega il motivo. Potresti avere più di un'organizzazione nel tuo account di prova esistente o applicazioni che non è possibile trasferire. Effettua l'operazione appropriata e prova a convertire di nuovo l'account.
{: tip}
...
Per riattivare le tue applicazioni, inizia di nuovo a utilizzarle nell'interfaccia riga di comando Cloud Foundry o nella console {{site.data.keyword.Bluemix_notm}}. 
...
Se la tua applicazione è già abilitata per SSH, non puoi utilizzare i comandi `cf enable-ssh` e `cf disable-sh` per riattivare la tua applicazione. Per ulteriori dettagli di utilizzo, vedi [Comandi di Cloud Foundry](/docs/cli/reference/cfcommands/index.html).
...
Se ricevi un invito da un amico o un da venditore {{site.data.keyword.Bluemix_notm}}, il tuo invito esclusivo viene inviato all'indirizzo email che hai fornito. Una volta ricevuto l'invito, completa le istruzioni riportate nell'e-mail per registrare un account standard. 
